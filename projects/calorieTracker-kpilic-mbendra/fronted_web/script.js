...
function render() {
    ...
    meals.forEach((m) => {
        // Filtriramo lokalno obrisane (hiddenMealIds) 
        // ali i one koji su na bazi označeni kao nevidljivi
        if (hiddenMealIds.includes(m.id) || m.is_visible === false) return;
        ...
        if(list) {
            ...
            li.innerHTML = `
                ...
                <button onclick="ukloniSamoJedan(${m.id})">×</button>
            `;
            list.appendChild(li);
        }
    });
...
async function obrisiSveZaDanas() {
    if(!confirm("Isprazniti dnevnik za danas?")) return;
    
    const email = localStorage.getItem('userEmail');
    const date = document.getElementById('date-picker').value;

    // Prvo vizualno sakrijemo
    meals.forEach(m => { if(m.id) hiddenMealIds.push(m.id); });
    render();

    // Zatim javimo bazi da ih sakrije trajno
    try {
        await fetch('http://127.0.0.1:5000/api/sakrij_obroke', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ email, date })
        });
    } catch (err) { console.error("Greška kod sinkronizacije brisanja:", err); }
}

window.ukloniSamoJedan = (id) => {
    hiddenMealIds.push(id);
    render();
    // Ovdje bi mogli dodati i API poziv za brisanje pojedinačnog jela
};
...
async function login() {
    ...
    try {
        ...
        if (result.success) ulazUAplikaciju(result.email, result.daily_kcal);
        else alert(result.message);
    } catch (err) { alert("Greška pri prijavi!"); }
}
...
function setManualGoal() {
    ...
    if(val > 0) { 
        dailyGoal = val; 
        localStorage.setItem('userDailyGoal', val); 
        prikaziDnevnik(); 
        // Spremamo cilj i na bazu
        fetch('http://127.0.0.1:5000/api/save_goal', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ email: localStorage.getItem('userEmail'), kcal: val })
        });
    }
}
    ...
    try {
        ...
        if(result.success) {
            ...
            await fetch('http://127.0.0.1:5000/api/save_goal', {
                ...
                body: JSON.stringify({ email: localStorage.getItem('userEmail'), kcal: dailyGoal })
            });
            prikaziDnevnik();
        }
    } catch (err) { alert("Greška pri izračunu!"); }
}
...
async function addTextMeal() {
    ...
    if(!name || !grams) return alert("Upiši naziv i grame!");

    try {
        // 1. Pitaj AI za nutritivne vrijednosti (Lookup)
        const res = await fetch('http://127.0.0.1:5000/api/lookup_meal', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ meal_name: name, grams: grams })
        });
        const data = await res.json();

        if(data.success) {
            // 2. Spremi rezultat u bazu
            await fetch('http://127.0.0.1:5000/api/add_meal', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                    user_email: localStorage.getItem('userEmail'),
                    meal: { ...data, category: category },
                    date: date
                })
            });
            document.getElementById('meal-name').value = '';
            document.getElementById('grams').value = ''
            loadUserMeals(); // Osvježi listu
        }
    } catch (err) { console.error("Greška kod ručnog unosa:", err); }
}
...
async function addImageMeal() {
    ...
    } catch (err) { console.error("Greška kod slike:", err); }
}