...
const fullPath = routesData?.fullPath || stations; // 1 hour 35 minutes

// Fetch highly detailed route from OSRM (like Google Maps)
const fetchRouteWithLRM = async (start, end) => {
    try {
        const [lat1, lng1] = start;
        const [lat2, lng2] = end;
        
        console.log(`\n📍 Route Request:`);
        console.log(`  From: [${lat1.toFixed(4)}, ${lng1.toFixed(4)}]`);
        console.log(`  To:   [${lat2.toFixed(4)}, ${lng2.toFixed(4)}]`);
        
        // CORRECT URL format: /route/v1/driving/lng1,lat1;lng2,lat2
        ...
        if (data.routes && data.routes[0]) {
            ...
            console.log(`  Duration: ${(route.duration / 60).toFixed(0)}min`);
            
            if (route.geometry && route.geometry.coordinates) {
                const coords = route.geometry.coordinates;
                console.log(`  Geometry points: ${coords.length}`);
                
                // OSRM returns [lng, lat], convert to [lat, lng]
                const roadPoints = coords.map(([lng, lat]) => [lat, lng]);
                
                console.log(`  First point: [${roadPoints[0][0].toFixed(4)}, ${roadPoints[0][1].toFixed(4)}]`);
                console.log(`  Last point: [${roadPoints[roadPoints.length-1][0].toFixed(4)}, ${roadPoints[roadPoints.length-1][1].toFixed(4)}]`);
                console.log(`✅ Route loaded with ${roadPoints.length} waypoints\n`);
                
                return roadPoints;
            }
        }
        
        console.log(`⚠️ No geometry found, using straight line fallback\n`);
        return [start, end];
