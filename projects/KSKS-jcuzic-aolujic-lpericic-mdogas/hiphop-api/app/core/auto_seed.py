...
def auto_fetch_albums():
    ...
    try:
        ...
        # If we have fewer than 10 albums, auto-fetch
        if album_count < 10:
            logger.info("📥 Auto-fetching albums from Spotify...")

            added_count = 0
            total_to_fetch = 0

            # Count total albums to fetch
            for genre_albums in AUTO_FETCH_ALBUMS.values():
                total_to_fetch += len(genre_albums)

            logger.info(f"🔍 Attempting to fetch {total_to_fetch} albums...")
                ...
                for album_name, artist_name, region in albums:
                    ...
                    try:
                        ...
                        if album_data:
                            ...
                            db.commit()
                            added_count += 1
                            logger.info(f"✅ Added: {album.artist} - {album.title}")

                    except Exception as e:
                        logger.warning(f"⚠️  Could not fetch {album_name}: {e}")
                        continue

            logger.info(f"🎉 Auto-fetch complete! Added {added_count} new albums")

        else:
            logger.info("✅ Database already populated, skipping auto-fetch")
