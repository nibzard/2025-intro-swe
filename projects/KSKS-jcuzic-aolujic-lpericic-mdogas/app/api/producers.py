...
@router.post("/{producer_id}/enrich-wikipedia", response_model=Producer)
def enrich_producer_wikipedia(producer_id: int, db: Session = Depends(get_db)):
    """
    Fetch Wikipedia biography for a producer
    """
    ...
    # Fetch Wikipedia bio
    wikipedia_bio = WikipediaService.get_producer_bio(
        producer_name=producer.name,
        sentences=10
    )
    
    if wikipedia_bio:
        producer.wikipedia_bio = wikipedia_bio
        db.commit()
        db.refresh(producer)
