          ...
          {analytics.categoryCounts.length === 0 ? (
            ...
            analytics.categoryCounts.map((category: any) => (
              <div key={category.name} className="flex items-center justify-between gap-3">
                <span className="text-sm sm:text-base text-gray-900 dark:text-white truncate flex-shrink min-w-0">{category.name}</span>
                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                  <div className="w-24 sm:w-48 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div
          ...
          <div className="space-y-3">
            {analytics.topTopics.length === 0 ? (
              ...
              analytics.topTopics.map((topic: any, index) => (
                <div
                  key={topic.slug}
                  className="flex items-start gap-2 sm:gap-3 pb-3 border-b border-gray-100 dark:border-gray-700 last:border-0"
                >
                  <div className="flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-semibold flex-shrink-0">
                    {index + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <a
                      href={`/forum/topic/${topic.slug}`}
                      className="text-xs sm:text-sm font-medium text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 line-clamp-2"
                    >
                      {topic.title}
                    </a>
