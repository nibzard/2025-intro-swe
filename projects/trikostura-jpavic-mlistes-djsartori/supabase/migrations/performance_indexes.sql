...
-- 1. Category browsing (most common user action)
-- 2. Topic viewing and reply loading
-- 3. User profile pages
-- 4. Notifications fetching (navbar + notifications page)
-- 5. Search and filtering operations
-- 6. Vote checking (prevent duplicates)
-- 7. Bookmark management

-- Expected performance improvements:
-- - 50-80% faster category page loads
-- - 60-90% faster topic page loads (eliminates N+1 on replies)
-- - 70-95% faster notification queries
-- - 80-95% faster user profile pages
-- - Instant bookmark checks

-- Maintenance:
-- - Indexes are automatically maintained by PostgreSQL
-- - Run ANALYZE periodically (once a week) to update statistics
-- - Monitor index usage with: SELECT * FROM pg_stat_user_indexes;
-- - Drop unused indexes if they're not being utilized
